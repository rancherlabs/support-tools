...
${KUBECTL_CMD} get clusters -o custom-columns=Cluster\ Id:metadata.name,Name:spec.displayName,K8s\ Version:status.version.gitVersion,Provider:status.driver,Created:metadata.creationTimestamp,Nodes:status.appliedSpec.rancherKubernetesEngineConfig.nodes[*].address

CLUSTER_IDS=$(${KUBECTL_CMD} get clusters --no-headers -o custom-columns=id:metadata.name)

for ID in $CLUSTER_IDS
do
  CLUSTER_NAME=$(${KUBECTL_CMD} get cluster ${ID} --no-headers -o custom-columns=name:spec.displayName)
