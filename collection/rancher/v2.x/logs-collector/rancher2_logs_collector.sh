...
K8S_OBJECTS_NAMESPACED=(apiservices apps.catalog.cattle.io configmaps cronjobs deployments daemonsets endpoints events helmcharts hpa ingress jobs leases networkpolicies networkpolicies.projectcalico.org pods pvc replicasets roles rolebindings statefulsets)
...
setup() {
  ...
  MKTEMP_ARGS=(-d)
  [ -n "${MKTEMP_BASEDIR[*]}" ] && MKTEMP_ARGS+=("${MKTEMP_BASEDIR[@]}")

  TMPDIR_BASE=$(mktemp "${MKTEMP_ARGS[@]}") || {
...
rke-logs() {
  ...
  docker images > "${TMPDIR}/docker/dockerimages" 2>&1 & timeout_cmd "docker_images"
...
rke-k8s() {
  ...
  [ -n "$SINCE_FLAG" ] && DOCKER_LOGS_CMD+=("$SINCE_FLAG")
  [ -n "$UNTIL_FLAG" ] && DOCKER_LOGS_CMD+=("$UNTIL_FLAG")
  ...
  for RANCHERSERVER in $RANCHERSERVERS; do
    ...
    "${DOCKER_LOGS_CMD[@]}" -t "$RANCHERSERVER" > "${TMPDIR}/rancher/containerlogs/server-$RANCHERSERVER" 2>&1
  done

  for RANCHERAGENT in $RANCHERAGENTS; do
    docker inspect "$RANCHERAGENT" > "${TMPDIR}/rancher/containerinspect/agent-$RANCHERAGENT" 2>&1
    "${DOCKER_LOGS_CMD[@]}" -t "$RANCHERAGENT" 2>&1 | sed 's/with token.*/with token REDACTED/g' > "${TMPDIR}/rancher/containerlogs/agent-$RANCHERAGENT" 2>&1
  done
  ...
  for KUBE_CONTAINER in "${KUBE_CONTAINERS[@]}"; do
    if [ "$(docker ps -a -q -f name="$KUBE_CONTAINER")" ]; then
      ...
      "${DOCKER_LOGS_CMD[@]}" -t "$KUBE_CONTAINER" > "${TMPDIR}/${DISTRO}/containerlogs/$KUBE_CONTAINER" 2>&1
    fi
  done
  ...
  for SYSTEM_NAMESPACE in "${SYSTEM_NAMESPACES[@]}"; do
    ...
    for CONTAINER in $CONTAINERS; do
      ...
      "${DOCKER_LOGS_CMD[@]}" -t "$CONTAINER" > "${TMPDIR}/${DISTRO}/podlogs/$CONTAINER" 2>&1
    done
  done
...
k3s-k8s() {
  ...
  if [ -d "/var/lib/rancher/${DISTRO}/agent" ]; then
    ...
    if ! k3s kubectl --kubeconfig="$KUBECONFIG" get --raw='/healthz' --request-timeout=5s > /dev/null 2>&1; then
...
rke2-k8s() {
  ...
  if [ -f "${RKE2_DATA_DIR}/agent/kubelet.kubeconfig" ]; then
    ...
    if ! "${RKE2_DATA_DIR}"/bin/kubectl --kubeconfig="$KUBECONFIG" get --raw='/healthz' --request-timeout=5s > /dev/null 2>&1; then
        API_SERVER_OFFLINE=true
        techo "[!] Kube-apiserver is offline, collecting local pod logs only"
    fi
  fi
  ...
  if find "${RKE2_DATA_DIR}/agent/pod-manifests" -mindepth 1 -print -quit | grep -q .; then
...
rke-certs() {
  ...
  if [ -d /opt/rke/etc/kubernetes/ssl ]; then
    CERTS=$(find /opt/rke/etc/kubernetes/ssl -type f -name '*.pem' ! -name '*-key.pem')
    ...
    if [ -d /opt/rke/etc/kubernetes/.tmp ]; then
      ...
      TMPCERTS=$(find /opt/rke/etc/kubernetes/.tmp -type f -name "*.pem" ! -name "*-key.pem")
      for TMPCERT in $TMPCERTS; do
        openssl x509 -in "$TMPCERT" -text -noout > "${TMPDIR}/${DISTRO}/tmpcerts/$(basename "$TMPCERT")" 2>&1
      done
    fi
  elif [ -d /etc/kubernetes/ssl ]; then
    CERTS=$(find /etc/kubernetes/ssl -type f -name '*.pem' ! -name '*-key.pem')
    for CERT in $CERTS; do
      openssl x509 -in "$CERT" -text -noout > "${TMPDIR}/${DISTRO}/certs/$(basename "$CERT")" 2>&1
    done
    if [ -d /etc/kubernetes/.tmp ]; then
      mkdir -p "${TMPDIR}/${DISTRO}/tmpcerts"
      TMPCERTS=$(find /etc/kubernetes/.tmp -type f -name '*.pem' ! -name '*-key.pem')
      for TMPCERT in $TMPCERTS; do
        openssl x509 -in "$TMPCERT" -text -noout > "${TMPDIR}/${DISTRO}/tmpcerts/$(basename "$TMPCERT")" 2>&1
      done
    fi
...
k3s-certs() {
  ...
  if [ -d /var/lib/rancher/k3s ]
    then
      ...
      AGENT_CERTS=$(find "/var/lib/rancher/${DISTRO}/agent" -maxdepth 1 -type f -name "*.crt" ! -name "*-ca.crt")
      ...
      if [ -d "/var/lib/rancher/${DISTRO}/server/tls" ]; then
        ...
        SERVER_CERTS=$(find "/var/lib/rancher/${DISTRO}/server/tls" -maxdepth 1 -type f -name "*.crt" ! -name "*-ca.crt")
...
kubeadm-certs() {
  ...
  if [ -d /etc/kubernetes/pki/ ]
    then
      ...
      SERVER_CERTS=$(find /etc/kubernetes/pki/ -maxdepth 2 -type f -name "*.crt" ! -name "*-ca.crt")
      for CERT in $SERVER_CERTS
        do
      ...
      if [ -d /var/lib/kubelet/pki/ ]; then
        ...
        AGENT_CERTS=$(find /var/lib/kubelet/pki/ -maxdepth 2 -type f -name "*.crt" ! -name "*-ca.crt")
        for CERT in $AGENT_CERTS
          do
...
rke2-certs() {
  ...
  if [ -d "${RKE2_DATA_DIR}" ]
    then
      ...
      mkdir -p "${TMPDIR}/${DISTRO}/certs/"{agent,server}
      AGENT_CERTS=$(find "${RKE2_DATA_DIR}/agent" -maxdepth 1 -type f -name "*.crt" ! -name "*-ca.crt")
      for CERT in $AGENT_CERTS
        do
          openssl x509 -in "$CERT" -text -noout > "${TMPDIR}/${DISTRO}/certs/agent/$(basename "$CERT")" 2>&1
      done
      if [ -d "${RKE2_DATA_DIR}/server/tls" ]; then
        ...
        SERVER_CERTS=$(find "${RKE2_DATA_DIR}/server/tls" -maxdepth 1 -type f -name "*.crt" ! -name "*-ca.crt")
        for CERT in $SERVER_CERTS
          do
            openssl x509 -in "$CERT" -text -noout > "${TMPDIR}/${DISTRO}/certs/server/$(basename "$CERT")" 2>&1
        done
...
archive() {
  ...
  if tar -czf "${DIR_NAME}/${LOGNAME}.tar.gz" -C "$TMPDIR_BASE" .
    then
