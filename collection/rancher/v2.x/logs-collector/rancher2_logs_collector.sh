...
# Included namespaces
SYSTEM_NAMESPACES=(kube-system kube-public cattle-system cattle-alerting cattle-logging cattle-pipeline ingress-nginx cattle-prometheus istio-system longhorn-system cattle-global-data fleet-system fleet-default rancher-operator-system cattle-monitoring-system cattle-logging-system cattle-fleet-system cattle-fleet-local-system tigera-operator calico-system)
...
help() {
  ...
  -s    Specify the time to start logging from container and journald logs. Specify the number of days before the current time.  (ex: -s 7)
  -e    Specify the time to end logging from container and journald logs. Specify the number of days before the current time.  (ex: -e 5)
