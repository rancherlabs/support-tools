...
if [ "$ENABLE_PRIVILEGED" = "true" ]; then
  CONTAINER_RUNTIME_ARGS+=(--privileged)
fi
...
if [ "${SONOBUOY_TOLERATION_FILE}" != "" ]; then
  ...
  CONTAINER_RUNTIME_ARGS+=(-v "${SONOBUOY_TOLERATION_FILE}:/tmp/sonobuoy_toleration.yml")
  COLLECT_INFO_FROM_RANCHER_SETUP_ARGS+=(--sonobuoy-toleration-file /tmp/sonobuoy_toleration.yml)
fi
...
if [ "${KUBECONFIG}" == "" ]; then
  ...
  CONTAINER_RUNTIME_ARGS+=(-e "RANCHER_URL=${RANCHER_URL}")
  CONTAINER_RUNTIME_ARGS+=(-e "RANCHER_TOKEN=${RANCHER_TOKEN}")
  CONTAINER_RUNTIME_ARGS+=(-e "RANCHER_VERIFY_SSL_CERTS=${RANCHER_VERIFY_SSL_CERTS}")
else
  ...
  CONTAINER_RUNTIME_ARGS+=(-v "${KUBECONFIG}:/tmp/kubeconfig.yml")
  COLLECT_INFO_FROM_RANCHER_SETUP_ARGS+=(--kubeconfig /tmp/kubeconfig.yml)

  if [ -d "$HOME/.aws" ]; then
    CONTAINER_RUNTIME_ARGS+=(-v "$HOME/.aws:/root/.aws")
  fi
  if [ -n "$AWS_ACCESS_KEY_ID" ] && [ -n "$AWS_SECRET_ACCESS_KEY" ]; then
    CONTAINER_RUNTIME_ARGS+=(-e "AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID}" -e "AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY}")
    if [ -n "$AWS_SESSION_TOKEN" ]; then
      CONTAINER_RUNTIME_ARGS+=(-e "AWS_SESSION_TOKEN=${AWS_SESSION_TOKEN}")
    fi
  fi

fi

CONTAINER_RUNTIME_ARGS+=(-e "BUNDLE_DIR=${BUNDLE_DIR}")
CONTAINER_RUNTIME_ARGS+=(-e "COLLECTOR_HTTP_PROXY=${COLLECTOR_HTTP_PROXY}")
CONTAINER_RUNTIME_ARGS+=(-e "COLLECTOR_HTTPS_PROXY=${COLLECTOR_HTTPS_PROXY}")
CONTAINER_RUNTIME_ARGS+=(-e "COLLECTOR_NO_PROXY=${COLLECTOR_NO_PROXY}")

CONTAINER_RUNTIME_ARGS+=("${SR_IMAGE}")
